...
global adj_network[adj_network.>1] .= 1; #changing to a 0 and 1 matrix
  ...
  N_p = potential_colonizers(adj_network, pollinators, plants, Splants); #number of potential colonizers
  N_i = length(total_island_species); #number of species in the island
  N_total = N_p + N_i + N_i; #total number

  dtotal = 1/N_total;

  c_event = N_p/N_total; #propability for having colonization
  coev_event = N_i/N_total; #propability for having coevolution
  ext_event = N_i/N_total; #propability for having extinction
